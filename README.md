...
**Data Scientist / Machine Learning Engineer (Python).** Ещё я: отец, муж и сын, коллега, старший по подъезду в многоквартирном доме. Несколько лет был председателем ревизионной комиссии в гаражном кооперативе на 600 человек.
...
10. **DB & cache:** `PostgreSQL`, `PGAdmin4`, `ClickHouse`[^in_progress], `Redis`[^in_progress].
11. **ETL scheduling:** `Apache Airflow`, `cron`.
12. **Event streaming:** `Apache Kafka`[^in_progress].
14. **Visualization and monitoring:** `Grafana`, `Graphite`, `Apache Superset`[^in_progress].
15. **OS:** `Linux` (`Ubuntu`), `Windows Server`, `Windows 10 Professional` + `WSL2`.
16. **Deploying Code:** `bare-metal`, `VPS`, `AWS`[^in_progress].
